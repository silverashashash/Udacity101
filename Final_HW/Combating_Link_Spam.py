...
def is_reciprical_link(graph, node, page, k): # if node->page is a reciprical link
    if k == 0:
        if node == page:
            return True
    if k == 1:
        if node in graph[page]:
            return True
    if k > 1:
        for element in graph[page]:
            if is_reciprical_link(graph, node, element, k-1):
                return True
    return False
        ...
        for page in graph:
            ...
            for node in graph:
                if page in graph[node]:
                    if not is_reciprical_link(graph, node, page, k):# if node->page is a reciprical link
